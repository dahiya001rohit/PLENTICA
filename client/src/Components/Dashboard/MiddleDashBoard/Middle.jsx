...
const Middle = ({ data, landSize }) => {
    ...
    const [donutGradient, setDonutGradient] = useState(
        `conic-gradient(#22c55e 0% 40%, #facc15 40% 70%, #3b82f6 70% 100%)`
    );
    ...
    useEffect(() => {
        if (data && Object.keys(data).length > 0) {
            ...
            const gradient = data.crops
                .map((crop) => {
                    const start = currentPercent;
                    const end = currentPercent + crop.percent;
                    currentPercent = end;
                    return `${crop.color} ${start}% ${end}%`;
                })
                .join(', ');

            setDonutGradient(`conic-gradient(${gradient})`);
        }
    }, [data]);
                    ...
                    {profits.map((crop) => (
                        <div key={crop.name} className='flex flex-col gap-1.5'>
                            <div className='flex items-center justify-between'>
                                ...
                                <span className='text-sm font-semibold text-gray-800'>{crop.amount || 'N/A'}</span>
                            </div>
                            <div className='w-full h-3 bg-gray-100 rounded-full overflow-hidden'>
                                <div
                                    className={`h-full ${crop.color || 'bg-gray-300'} rounded-full`}
                                    style={{ width: `${crop.percent || 0}%` }}
                                />
                            </div>
                        </div>
                    ))}
