...
const RightTop = () => {
  return (
    <div className='w-full h-[40%] bg-white rounded-xl border border-gray-200 p-5 flex flex-col'>
      {/* Header */}
      ...
      <div className='flex flex-col gap-4 flex-1'>
        {insights.map((item) => (
          <div key={item.title} className='flex items-start gap-3'>
            <div className={`w-7 h-7 rounded-full flex items-center justify-center text-sm flex-shrink-0 ${item.iconBg}`}>
              {item.icon}
            </div>
